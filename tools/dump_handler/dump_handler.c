...
static void _zip_fw_files (const char *storage_path, int card_idx, BOOL no_limit_dumps){
  ...
  char                old_dump_filename[MAX_FILE_NAME_SIZE] = {0};
  ...
  snprintf_res = snprintf(tmp_dump_dir_full_path,
  ...
  if (pf && (fgets(out_str, MAX_FILE_NAME_SIZE , pf) != NULL)){
    stat = pclose(pf);
    pf = NULL;
    if (WEXITSTATUS(stat) == 0 && *out_str){
      if (sscanf(out_str, "%d %s\r", &old_dump_size, old_dump_filename) == 2){
  ...
  if (single_dump_file &&
      ...
      (free_space_in_fs < (old_dump_size + LEAVE_FREE_SPACE_ON_FS_IN_KB * 1024))){
    if (difftime(curr_time, time_last_dump_saved) > DUMP_FILE_AGING_TIME_IN_SEC){
      if (remove (old_dump_filename) != 0)
        ELOG_S("Removing old dump file %s failed", old_dump_filename);
      else
        ELOG_S("Removed old dump file %s", old_dump_filename);
    }
    goto save_dumps;
  }
...
save_dumps:
  ...
  if (get_free_space (storage_path, &free_space_in_fs) == MTLK_ERR_OK){
    ...
    if (free_space_in_fs <= (LEAVE_FREE_SPACE_ON_FS_IN_KB * 1024)){
      ...
      snprintf_res = snprintf(out_str, sizeof (out_str), "%s/%s",
                              storage_path, tar_file_name);
      if (snprintf_res <= 0 || snprintf_res >= sizeof (out_str)){
