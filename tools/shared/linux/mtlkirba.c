...
#define NO_BY_SON(son) ((long int)(son) - 1)
                        ...
                        mtlk_node_son_t *son)
{
  ...
  snprintf(key, sizeof(key), MTLK_IRB_INI_SON_PREFIX "%ld", son_no);
...
_mtlk_irba_get_fname (char *buffer, size_t size, const char* unique_desc)
{
  ...
  res = snprintf(buffer, size, "%s/mtlk_%s_%u_%u", path, unique_desc, pid, ctr);
  if (res < 0)
    return MTLK_ERR_PARAMS;
  if (res >= size)
    return MTLK_ERR_BUF_TOO_SMALL;
